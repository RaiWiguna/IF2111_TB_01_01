...
Address Allocate(ElementType X) {
    ...
    if (P != NIL) {
        InfoL(P) = (char*)malloc((strlen_custom(X) + 1) * sizeof(char));
        if (InfoL(P) != NIL) {
            *strcpy_custom(InfoL(P), X);
            NextL(P) = NIL;
            PrevL(P) = NIL;
        } else {
...
void DeleteNode(LinkedList *L, ElementType X) {
    ...
    if (P == NIL) {
        printf("Node dengan nilai %s tidak ditemukan.\n", X);
        return;
    }
    ...
    // Jika node yang ditemukan adalah node terakhir
    if (P == LastL(*L)) {
        Address temp;
        DeleteLastL(L, &temp);
        Deallocate(temp);
    ...
    Address prevNode = PrevL(P);
    Address nextNode = NextL(P);

    if (prevNode != NIL) {
        NextL(prevNode) = nextNode;
    }
    if (nextNode != NIL) {
        PrevL(nextNode) = prevNode;
    }

    Deallocate(P);
    printf("Node dengan nilai %s berhasil dihapus.\n", X);
}
